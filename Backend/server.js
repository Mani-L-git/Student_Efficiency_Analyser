...
const isPhone = v => /^[6-9]\d{9}$/.test(String(v));
...
async function runMigrations() {
  const alterIfMissing = async (table, column, definition) => {
    try {
      ...
      console.warn(`Migration skipped for ${table}.${column}:`, e.message);
    }
  };
  await alterIfMissing("users", "phone",     "VARCHAR(15)  DEFAULT NULL");
  await alterIfMissing("users", "is_active", "TINYINT(1)   NOT NULL DEFAULT 1");
  await alterIfMissing("users", "left_date", "DATE         DEFAULT NULL");
}
...
app.post("/superadmin/department", verifyToken, verifySuperAdmin, async (req, res) => {
  const { name } = req.body;
  if (!name?.trim()) return res.status(400).json({ message: "Department name required" });
...
app.post("/superadmin/add-admin", verifyToken, verifySuperAdmin,
  validate({ name: "string", email: "string", password: "string", department: "string", phone: "string" }),
  async (req, res) => {
    const { name, email, password, department, phone } = req.body;
    if (!isEmail(email))     return res.status(400).json({ message: "Invalid email format" });
    if (password.length < 6) return res.status(400).json({ message: "Password must be at least 6 characters" });
    if (!isPhone(phone))     return res.status(400).json({ message: "Invalid phone number (10 digits, starts 6-9)" });
    try {
      ...
      await pool.query(
        "INSERT INTO users (name, email, password, role, department, phone, is_active) VALUES (?,?,?,'admin',?,?,1)",
...
app.get("/superadmin/admins", verifyToken, verifySuperAdmin, async (req, res) => {
  try {
    const [r] = await pool.query(
      "SELECT id, name, email, phone, department, is_active, left_date, created_at FROM users WHERE role='admin' ORDER BY name"
    );
    res.json(r);
  } catch { res.status(500).json({ message: "Server error" }); }
});

app.put("/superadmin/admin/:id", verifyToken, verifySuperAdmin, async (req, res) => {
  const { name, email, phone, department, is_active, left_date } = req.body;
  if (!name?.trim())   return res.status(400).json({ message: "Name required" });
  if (!isEmail(email)) return res.status(400).json({ message: "Valid email required" });
  if (!isPhone(phone)) return res.status(400).json({ message: "Valid 10-digit phone required" });
  if (!department)     return res.status(400).json({ message: "Department required" });
  try {
    const [ex] = await pool.query("SELECT id FROM users WHERE email=? AND id!=?", [email, req.params.id]);
    if (ex.length) return res.status(400).json({ message: "Email already in use by another account" });
    const activeVal   = is_active ? 1 : 0;
    const leftDateVal = !is_active && left_date ? left_date : null;
    await pool.query(
      "UPDATE users SET name=?, email=?, phone=?, department=?, is_active=?, left_date=? WHERE id=? AND role='admin'",
      [name, email, phone, department, activeVal, leftDateVal, req.params.id]
    );
...
app.get("/superadmin/dept-perf-counts", verifyToken, verifySuperAdmin, async (req, res) => {
  try {
    ...
    for (const r of mk) {
      ...
      if (!cgpaMap[k]) cgpaMap[k] = { w: 0, c: 0 };
      cgpaMap[k].w += Number(r.grade_points) * Number(r.credits);
      cgpaMap[k].c += Number(r.credits);
    }
    ...
    for (const s of students) {
      ...
      if (!dept) continue;
      const skS = maxSk  > 0 ? ((skillMap[id] || 0) / maxSk)  * 100 : 0;
      const acS = maxAch > 0 ? ((achMap[id]   || 0) / maxAch) * 100 : 0;
      const atS = maxAct > 0 ? ((actMap[id]   || 0) / maxAct) * 100 : 0;
      const cd  = cgpaMap[id];
      const cgS = cd && cd.c > 0 ? Math.min((cd.w / cd.c / 10) * 100, 100) : 0;
      const band = getBand(skS * 0.30 + acS * 0.20 + atS * 0.20 + cgS * 0.30);
      if (!result[dept]) result[dept] = { Excellent: 0, Good: 0, "Need Improvement": 0, Weak: 0 };
      result[dept][band]++;
    }
    res.json(result);
  } catch (e) { console.error(e); res.status(500).json({ message: "Server error" }); }
...
app.get("/superadmin/dept-efficiency", verifyToken, verifySuperAdmin, async (req, res) => {
  try {
    ...
    for (const s of students) {
      ...
      const cg = cd && cd.c > 0 ? Math.min((cd.w / cd.c / 10) * 100, 100) : 0;
      deptMap[s.department].total += sk * 0.30 + ac * 0.20 + at * 0.20 + cg * 0.30;
      deptMap[s.department].count++;
    }
    res.json(Object.entries(deptMap).map(([dept, { total, count }]) => ({
      department: dept,
...
app.post("/superadmin/announcement", verifyToken, verifySuperAdmin, async (req, res) => {
  ...
  try {
    await pool.query("INSERT INTO announcements (title,message,target) VALUES (?,?,?)", [title, message, target || "all"]);
...
app.post("/admin/announcement", verifyToken, verifyAdmin, async (req, res) => {
  ...
  try {
    await pool.query("INSERT INTO announcements (title,message,target) VALUES (?,?,?)", [title, message, req.user.department]);
    res.json({ message: "Posted to " + req.user.department });
  } catch { res.status(500).json({ message: "Server error" }); }
});
...
app.post("/announcement/:id/reply", verifyToken, async (req, res) => {
  ...
  try {
    await pool.query(
      ...
      [req.params.id, req.user.id, reply_text.trim(), is_voice ? 1 : 0]
    );
...
app.post("/add-student", verifyToken, verifyAdmin,
  ...
  async (req, res) => {
    ...
    if (!isEmail(email))          return res.status(400).json({ message: "Invalid email format" });
    if (password.length < 4)      return res.status(400).json({ message: "Password min 4 chars" });
    if (phone && !isPhone(phone)) return res.status(400).json({ message: "Invalid phone number (10 digits)" });
    ...
    try {
      ...
      await pool.query(
        "INSERT INTO users (name, rollno, email, password, role, department, phone, is_active) VALUES (?,?,?,?,'student',?,?,1)",
        [name, rollno, email, password, dept, phone || null]
      );
...
app.get("/students", verifyToken, verifyFaculty, async (req, res) => {
  try {
    ...
    if (req.user.role === "superadmin") {
      [r] = await pool.query(
        "SELECT id, name, rollno, email, department, phone, is_active FROM users WHERE role='student' ORDER BY department, name"
      );
    } else {
      [r] = await pool.query(
        "SELECT id, name, rollno, email, department, phone, is_active FROM users WHERE role='student' AND department=? ORDER BY name",
        [req.user.department]
      );
    }
    res.json(r);
...
app.get("/student/:id", verifyToken, async (req, res) => {
  try {
    const [r] = await pool.query(
      "SELECT id, name, rollno, email, department, phone, is_active FROM users WHERE id=? AND role='student'",
      [req.params.id]
    );
...
app.put("/admin/student/:id", verifyToken, verifyAdmin, async (req, res) => {
  const { name, email, phone, is_active } = req.body;
  if (!name?.trim())   return res.status(400).json({ message: "Name required" });
  if (!isEmail(email)) return res.status(400).json({ message: "Valid email required" });
  if (phone && !isPhone(phone)) return res.status(400).json({ message: "Invalid phone number (10 digits, starts 6-9)" });
  const studentId = req.params.id;
  try {
    if (req.user.role !== "superadmin") {
      const [r] = await pool.query("SELECT department FROM users WHERE id=? AND role='student'", [studentId]);
      if (!r.length) return res.status(404).json({ message: "Student not found" });
      if (r[0].department !== req.user.department)
        return res.status(403).json({ message: "Cannot edit student from another department" });
    }
    const [ex] = await pool.query("SELECT id FROM users WHERE email=? AND id!=?", [email, studentId]);
    if (ex.length) return res.status(400).json({ message: "Email already in use by another account" });
    const activeVal = is_active ? 1 : 0;
    await pool.query(
      "UPDATE users SET name=?, email=?, phone=?, is_active=? WHERE id=? AND role='student'",
      [name.trim(), email.trim(), phone || null, activeVal, studentId]
    );
...
app.delete("/student/:id", verifyToken, verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    await pool.query("DELETE FROM student_activities   WHERE student_id=?", [studentId]);
...
app.post("/admin/bulk-add-students", verifyToken, verifyAdmin, async (req, res) => {
  ...
  const results = [];
  for (const s of students) {
    const { name, rollno, email, password, phone } = s;
    if (!name || !rollno || !email || !password) {
      results.push({ rollno: rollno || "?", status: "error", message: "Missing fields" }); continue;
    }
    ...
    try {
      ...
      if (eEx.length) { results.push({ rollno, status: "error", message: "Email exists" }); continue; }
      const [rEx] = await pool.query("SELECT id FROM users WHERE rollno=?", [rollno]);
      if (rEx.length) { results.push({ rollno, status: "error", message: "Roll no exists" }); continue; }
      await pool.query(
        "INSERT INTO users (name, rollno, email, password, role, department, phone, is_active) VALUES (?,?,?,?,'student',?,?,1)",
        [name.trim(), rollno.trim(), email.trim(), password.trim(), dept, phone || null]
      );
...
app.get("/admin/faculty", verifyToken, verifyAdmin, async (req, res) => {
  try {
    const [rows] = req.user.role === "superadmin"
      ? await pool.query("SELECT id, name, email, department, phone, is_active, created_at FROM users WHERE role='faculty' ORDER BY name")
      : await pool.query("SELECT id, name, email, department, phone, is_active, created_at FROM users WHERE role='faculty' AND department=? ORDER BY name", [req.user.department]);
    res.json(rows);
  } catch (e) { console.error(e); res.status(500).json({ message: "Server error" }); }
});
...
app.post("/admin/add-faculty", verifyToken, verifyAdmin,
  ...
  async (req, res) => {
    ...
    if (!isEmail(email))          return res.status(400).json({ message: "Invalid email format" });
    if (password.length < 4)      return res.status(400).json({ message: "Password min 4 chars" });
    if (phone && !isPhone(phone)) return res.status(400).json({ message: "Invalid phone number" });
    ...
    try {
      ...
      await pool.query(
        "INSERT INTO users (name, email, password, role, department, phone) VALUES (?,?,?,'faculty',?,?)",
        [name, email, password, department, phone || null]
      );
      const [newRow] = await pool.query(
        "SELECT id, name, email, department, phone FROM users WHERE email=? AND role='faculty'", [email]
      );
...
app.delete("/admin/faculty/:id", verifyToken, verifyAdmin, async (req, res) => {
  try {
    if (req.user.role !== "superadmin") {
      const [rows] = await pool.query("SELECT department FROM users WHERE id=? AND role='faculty'", [req.params.id]);
...
app.put("/admin/faculty/:id", verifyToken, verifyAdmin, async (req, res) => {
  const { name, email, phone, is_active } = req.body;
  if (!name?.trim())   return res.status(400).json({ message: "Name required" });
  if (!isEmail(email)) return res.status(400).json({ message: "Valid email required" });
  if (phone && !isPhone(phone)) return res.status(400).json({ message: "Invalid phone number (10 digits)" });
  try {
    if (req.user.role !== "superadmin") {
      const [rows] = await pool.query(
        "SELECT department FROM users WHERE id=? AND role='faculty'", [req.params.id]
      );
      if (!rows.length) return res.status(404).json({ message: "Faculty not found" });
      if (rows[0].department !== req.user.department)
        return res.status(403).json({ message: "Cannot edit faculty from another department" });
    }
    ...
    await pool.query(
      "UPDATE users SET name=?, email=?, phone=?, is_active=? WHERE id=? AND role='faculty'",
      [name.trim(), email.trim(), phone || null, activeVal, req.params.id]
    );
...
app.post("/faculty/note", verifyToken, async (req, res) => {
  ...
  if (!student_id || !note?.trim()) return res.status(400).json({ message: "student_id and note required" });
  try {
    await pool.query(
...
app.get("/faculty/notes/:student_id", verifyToken, async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      `SELECT n.id, n.note, n.created_at, u.name as faculty_name
       FROM faculty_notes n
       JOIN users u ON n.faculty_id = u.id
       WHERE n.student_id=? AND n.faculty_id=?
...
app.put("/admin/reset-faculty-password", verifyToken, verifyAdmin, async (req, res) => {
  const { faculty_id, new_password } = req.body;
  if (!faculty_id || !new_password) return res.status(400).json({ message: "faculty_id and new_password required" });
  if (new_password.length < 4) return res.status(400).json({ message: "Password min 4 chars" });
  try {
    if (req.user.role !== "superadmin") {
      const [r] = await pool.query("SELECT department FROM users WHERE id=? AND role='faculty'", [faculty_id]);
      if (!r.length) return res.status(404).json({ message: "Faculty not found" });
      if (r[0].department !== req.user.department)
        return res.status(403).json({ message: "Cannot reset password for faculty in another department" });
    }
    await pool.query("UPDATE users SET password=? WHERE id=? AND role='faculty'", [new_password, faculty_id]);
...
app.put("/change-password", verifyToken, async (req, res) => {
  ...
  if (new_password.length < 4) return res.status(400).json({ message: "New password min 4 chars" });
  try {
...
app.get("/subjects", verifyToken, async (req, res) => {
  try {
    let r;
    if (req.user.role === "superadmin") {
      [r] = await pool.query("SELECT * FROM subjects ORDER BY department, subject_name");
    } else if (req.user.role === "admin") {
      [r] = await pool.query("SELECT * FROM subjects WHERE department=? ORDER BY subject_name", [req.user.department]);
    } else {
      const [u] = await pool.query("SELECT department FROM users WHERE id=?", [req.user.id]);
      [r] = await pool.query("SELECT * FROM subjects WHERE department=? ORDER BY subject_name", [u[0]?.department]);
    }
    res.json(r);
  } catch { res.status(500).json({ message: "Server error" }); }
});
...
app.post("/add-subject", verifyToken, verifyAdmin, validate({ subject_name: "string", credits: "number" }), async (req, res) => {
  ...
  if (cr < 1 || cr > 5) return res.status(400).json({ message: "Credits must be 1-5" });
  const dept = req.user.department;
  if (!dept) return res.status(403).json({ message: "Admin has no department assigned" });
  try {
    ...
    await pool.query("INSERT INTO subjects (subject_name, department, credits, semester) VALUES (?,?,?,?)",
      [subject_name, dept, cr, semester || "Sem 1"]);
...
app.get("/subjects-by-sem", verifyToken, async (req, res) => {
  try {
    let r;
    if (req.user.role === "superadmin") {
      [r] = await pool.query("SELECT * FROM subjects ORDER BY semester, subject_name");
    } else if (req.user.role === "admin") {
      [r] = await pool.query("SELECT * FROM subjects WHERE department=? ORDER BY semester, subject_name", [req.user.department]);
    } else {
      const [u] = await pool.query("SELECT department FROM users WHERE id=?", [req.user.id]);
      [r] = await pool.query("SELECT * FROM subjects WHERE department=? ORDER BY semester, subject_name", [u[0]?.department]);
    }
    res.json(r);
  } catch { res.status(500).json({ message: "Server error" }); }
});
...
app.post("/add-marks", verifyToken, verifyAdmin,
  ...
  async (req, res) => {
    ...
    if (marks < 0 || marks > 100) return res.status(400).json({ message: "Marks must be 0-100" });
    ...
    try {
      ...
      if (dup.length) return res.status(400).json({ message: "Marks already entered for this subject and semester" });
      await pool.query(
        "INSERT INTO marks (student_id, subject_id, marks_scored, grade, grade_points, semester, department) VALUES (?,?,?,?,?,?,?)",
        [student_id, subject_id, marks, grade, gradePoints, semester, req.user.department]
      );
      const [sm] = await pool.query(
        "SELECT m.grade_points, s.credits FROM marks m JOIN subjects s ON m.subject_id=s.id WHERE m.student_id=? AND m.semester=?",
        [student_id, semester]
      );
      const tc = sm.reduce((s, r) => s + r.credits, 0);
      const ws = sm.reduce((s, r) => s + r.grade_points * r.credits, 0);
...
app.put("/mark/:id", verifyToken, verifyAdmin, async (req, res) => {
  const { marks_scored } = req.body;
  if (marks_scored === undefined || marks_scored === null)
    return res.status(400).json({ message: "marks_scored required" });
  const marks = Number(marks_scored);
  ...
  try {
    if (req.user.role !== "superadmin") {
      ...
      if (!r.length) return res.status(404).json({ message: "Mark not found" });
      if (r[0].department !== req.user.department)
        return res.status(403).json({ message: "Cannot edit mark from another department" });
    }
    await pool.query(
      "UPDATE marks SET marks_scored=?, grade=?, grade_points=? WHERE id=?",
      [marks, grade, gradePoints, req.params.id]
    );
    res.json({ message: `Marks updated - Grade: ${grade}`, grade, gradePoints });
  } catch (e) { console.error(e); res.status(500).json({ message: "Server error" }); }
});

app.get("/all-marks", verifyToken, verifyFaculty, async (req, res) => {
  try {
    const base = `SELECT m.*, s.subject_name, s.credits, u.name as student_name
                  FROM marks m
...
app.get("/student-marks/:id", verifyToken, async (req, res) => {
  try {
    const [rows] = await pool.query(
      "SELECT m.*, s.subject_name, s.credits FROM marks m JOIN subjects s ON m.subject_id=s.id WHERE m.student_id=? ORDER BY m.semester",
      [req.params.id]
    );
    const semMap = {};
    for (const r of rows) {
    ...
    const semesters = Object.keys(semMap).map(sem => {
      const { subjects, totalCredits, weightedSum } = semMap[sem];
      return { semester: sem, sgpa: totalCredits > 0 ? (weightedSum / totalCredits).toFixed(2) : "0.00", subjects };
    });
    const ac = rows.reduce((s, r) => s + Number(r.credits), 0);
    const aw = rows.reduce((s, r) => s + Number(r.grade_points) * Number(r.credits), 0);
...
app.post("/admin/bulk-add-marks", verifyToken, verifyAdmin, async (req, res) => {
  ...
  for (const row of rows) {
    ...
    if (!roll_no || !semester || !subject || marks === undefined || marks === "") {
      results.push({ roll_no, status: "error", message: "Missing fields" }); continue;
    }
    const m = Number(marks);
    if (isNaN(m) || m < 0 || m > 100) { results.push({ roll_no, status: "error", message: "Marks 0-100" }); continue; }
    try {
      const norm = String(roll_no).replace(/\s+/g, "").toUpperCase();
      const [stu] = await pool.query(
        "SELECT id FROM users WHERE UPPER(REPLACE(rollno,' ',''))=? AND department=? AND role='student'",
        [norm, dept]
      );
      if (!stu.length) { results.push({ roll_no, status: "error", message: `Roll not found in ${dept}` }); continue; }
      ...
      if (!matched) matched = subs.find(s => cn(s.subject_name) === cn(subject));
      if (!matched) matched = subs.find(s => cn(s.subject_name).includes(cn(subject)) || cn(subject).includes(cn(s.subject_name)));
      if (!matched) { results.push({ roll_no, status: "error", message: `Subject "${subject}" not found` }); continue; }
      const { grade, gradePoints } = calculateGrade(m);
      const [dup] = await pool.query(
        "SELECT id FROM marks WHERE student_id=? AND subject_id=? AND semester=?",
        [studentId, matched.id, semester]
      );
      if (dup.length) {
        await pool.query(
          "UPDATE marks SET marks_scored=?, grade=?, grade_points=? WHERE student_id=? AND subject_id=? AND semester=?",
          [m, grade, gradePoints, studentId, matched.id, semester]
        );
        results.push({ roll_no, status: "ok", message: `Updated - ${grade}` });
      } else {
        await pool.query(
          "INSERT INTO marks (student_id, subject_id, marks_scored, grade, grade_points, semester, department) VALUES (?,?,?,?,?,?,?)",
          [studentId, matched.id, m, grade, gradePoints, semester, dept]
        );
...
app.get("/student-attendance/:id", verifyToken, async (req, res) => {
  try {
    const [r] = await pool.query("SELECT * FROM attendance WHERE student_id=? ORDER BY semester", [req.params.id]);
    res.json(r);
  } catch (e) { console.error(e); res.status(500).json({ message: "Server error" }); }
});
...
app.post("/add-attendance", verifyToken, verifyAdmin,
  ...
  async (req, res) => {
    ...
    if (p < 0 || t <= 0) return res.status(400).json({ message: "Invalid day values" });
    if (p > t)           return res.status(400).json({ message: "Present days cannot exceed total" });
    const pct = parseFloat(((p / t) * 100).toFixed(2));
    try {
      await pool.query(
        `INSERT INTO attendance (student_id, semester, present_days, total_days, attendance_percentage)
         VALUES (?,?,?,?,?)
         ON DUPLICATE KEY UPDATE
           present_days          = VALUES(present_days),
           total_days            = VALUES(total_days),
           attendance_percentage = VALUES(attendance_percentage)`,
        [student_id, semester, p, t, pct]
      );
...
app.get("/attendance-list", verifyToken, verifyFaculty, async (req, res) => {
  try {
    const base = `SELECT a.*, u.name as student_name, u.rollno, u.department
                  FROM attendance a
...
app.get("/efficiency/:studentId", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const calcFinal = id => {
      const s  = String(id);
      const sk = maxSkill > 0 ? ((skillMap[s] || 0) / maxSkill) * 100 : 0;
      const ac = maxAch   > 0 ? ((achMap[s]   || 0) / maxAch)  * 100 : 0;
      const at = maxAct   > 0 ? ((actMap[s]   || 0) / maxAct)  * 100 : 0;
      const cd = cgpaMap[s];
      const cg = cd && cd.c > 0 ? Math.min((cd.w / cd.c / 10) * 100, 100) : 0;
    ...
    res.json({
      ...
      deptPercentile: {
        skill:       percentile(mySkillRaw, deptIds.map(id => skillMap[String(id)] || 0)),
        achievement: percentile(myAchRaw,   deptIds.map(id => achMap[String(id)]   || 0)),
        activity:    percentile(myActRaw,   deptIds.map(id => actMap[String(id)]   || 0)),
        cgpa:        percentile(cgpa,       deptIds.map(id => { const d = cgpaMap[String(id)]; return d && d.c > 0 ? d.w / d.c : 0; })),
        overall:     percentile(finalScore, deptFinalScores),
      },
      allPercentile: {
        skill:       percentile(mySkillRaw, allIds.map(id => skillMap[String(id)] || 0)),
        achievement: percentile(myAchRaw,   allIds.map(id => achMap[String(id)]   || 0)),
        activity:    percentile(myActRaw,   allIds.map(id => actMap[String(id)]   || 0)),
        cgpa:        percentile(cgpa,       allIds.map(id => { const d = cgpaMap[String(id)]; return d && d.c > 0 ? d.w / d.c : 0; })),
        overall:     percentile(finalScore, allFinalScores),
      },
      deptRank:     deptFinalScores.filter(s => s > finalScore).length + 1,
      deptTotal:    deptIds.length,
      overallRank:  allFinalScores.filter(s => s > finalScore).length + 1,
      overallTotal: allIds.length,
    });
  } catch (e) { console.error(e); res.status(500).json({ message: "Server error" }); }
});
...
app.get("/admin/all-efficiency", verifyToken, verifyFaculty, async (req, res) => {
  try {
    ...
    const disp = req.user.role === "superadmin"
      ? allStudents
    ...
    for (const r of mk) {
      const k = String(r.student_id);
      if (!cmap[k]) cmap[k] = { w: 0, c: 0 };
      cmap[k].w += Number(r.grade_points) * Number(r.credits);
      cmap[k].c += Number(r.credits);
    }

    const calcF = id => {
      const s   = String(id);
      const sk2 = maxSk  > 0 ? ((skillMap[s] || 0) / maxSk)  * 100 : 0;
      const ac  = maxAch > 0 ? ((achMap[s]   || 0) / maxAch) * 100 : 0;
      const at  = maxAct > 0 ? ((actMap[s]   || 0) / maxAct) * 100 : 0;
      const cd  = cmap[s];
      const cg  = cd && cd.c > 0 ? Math.min((cd.w / cd.c / 10) * 100, 100) : 0;
      return { sk: sk2, ac, at, cg, final: sk2 * 0.30 + ac * 0.20 + at * 0.20 + cg * 0.30 };
    };

    const allScoreMap = Object.fromEntries(allStudents.map(s => [s.id, calcF(s.id).final]));
    const cgpaVM      = Object.fromEntries(allStudents.map(x => {
      const c = cmap[String(x.id)];
      return [String(x.id), c && c.c > 0 ? c.w / c.c : 0];
    }));
    const pctFn = (mv, ids, map) => {
      const vals = ids.map(id => map[String(id)] || 0);
      if (!vals.length) return 0;
      return Math.round((vals.filter(v => v < mv).length / vals.length) * 100);
    };

    const results = disp.map(s => {
      const { sk, ac, at, cg, final } = calcF(s.id);
      const cd   = cmap[String(s.id)];
      ...
      return {
        ...
        skillScore:       Math.round(sk),
        achievementScore: Math.round(ac),
        activityScore:    Math.round(at),
        cgpaScore:        Math.round(cg),
        finalScore:       Math.round(final * 10) / 10,
        cgpa:             cgpa.toFixed(2),
        band: final >= 80 ? "Excellent" : final >= 60 ? "Good" : final >= 40 ? "Needs Improvement" : "Weak",
        overallRank:  Object.values(allScoreMap).filter(v => v > final).length + 1,
        overallTotal: allStudents.length,
        deptRank:  deptIds.filter(id => (allScoreMap[id] || 0) > final).length + 1,
        deptTotal: deptIds.length,
        deptPercentile: {
          skill:       pctFn(skillMap[String(s.id)] || 0, deptIds, skillMap),
          achievement: pctFn(achMap[String(s.id)]   || 0, deptIds, achMap),
          activity:    pctFn(actMap[String(s.id)]   || 0, deptIds, actMap),
          cgpa:        pctFn(cgpa, deptIds, cgpaVM),
          overall:     Math.round((deptIds.filter(id => (allScoreMap[id] || 0) < final).length / deptIds.length) * 100),
        },
        allPercentile: {
          skill:       pctFn(skillMap[String(s.id)] || 0, allStudents.map(x => x.id), skillMap),
          achievement: pctFn(achMap[String(s.id)]   || 0, allStudents.map(x => x.id), achMap),
          activity:    pctFn(actMap[String(s.id)]   || 0, allStudents.map(x => x.id), actMap),
          cgpa:        pctFn(cgpa, allStudents.map(x => x.id), cgpaVM),
...
app.post("/admin/student-skill", verifyToken, verifyAdmin, async (req, res) => {
  ...
  try {
    await pool.query(
      "INSERT INTO student_skills (student_id, skill_level, skill_score) VALUES (?,?,?) ON DUPLICATE KEY UPDATE skill_level=VALUES(skill_level), skill_score=VALUES(skill_score)",
...
app.post("/admin/student-activity", verifyToken, verifyAdmin, async (req, res) => {
  ...
  try {
    await pool.query(
      "INSERT INTO student_activities (student_id, activity_type, points, description, added_by) VALUES (?,?,?,?,?)",
...
app.put("/admin/student-achievement/:id", verifyToken, verifyAdmin, async (req, res) => {
  ...
  if (!achievement_name || points === undefined)
    return res.status(400).json({ message: "achievement_name and points required" });
  try {
    await pool.query(
...
app.get("/admin/student-efficiency-data/:studentId", verifyToken, verifyAdmin, async (req, res) => {
  try {
    const [skillRows] = await pool.query("SELECT * FROM student_skills       WHERE student_id=?",                         [req.params.studentId]);
    const [actRows]   = await pool.query("SELECT * FROM student_activities   WHERE student_id=? ORDER BY created_at DESC", [req.params.studentId]);
    const [achRows]   = await pool.query("SELECT * FROM student_achievements WHERE student_id=? ORDER BY created_at DESC", [req.params.studentId]);
